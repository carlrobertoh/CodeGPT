...
public class TelemetryService implements ITelemetryService {
    ...
    private void onConfigurationChanged(MessageBusConnection connection) {
        connection.subscribe(TelemetryConfiguration.ConfigurationChangedListener.CONFIGURATION_CHANGED, (String key, String value) -> {
            if (TelemetryConfiguration.KEY_MODE.equals(key)
                    && TelemetryConfiguration.Mode.safeValueOf(value).isEnabled()) {
                flushOnHold();
            }
        });
    }
